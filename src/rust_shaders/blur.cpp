#include "tekki/rust_shaders/blur.h"
#include <glm/glm.hpp>
#include <glm/gtc/type_ptr.hpp>
#include <vector>
#include <memory>
#include <cmath>
#include <stdexcept>

namespace tekki::rust_shaders {

float Blur::GaussianWt(float dstPx, float srcPx) {
    float pxOff = (dstPx + 0.5f) * 2.0f - (srcPx + 0.5f);
    float sigma = static_cast<float>(KERNEL_RADIUS) * 0.5f;
    return std::exp(-pxOff * pxOff / (sigma * sigma));
}

glm::vec4 Blur::VBlur(const std::shared_ptr<void>& inputTex, const glm::ivec2& dstPx, const glm::ivec2& srcPx) {
    glm::vec4 res(0.0f);
    float wtSum = 0.0f;

    int32_t y = 0;
    while (y < KERNEL_RADIUS * 2) {
        try {
            float wt = GaussianWt(static_cast<float>(dstPx.y), static_cast<float>(srcPx.y + y));
            glm::vec4 value = FetchTexture(inputTex, srcPx + glm::ivec2(0, y));
            res += value * wt;
            wtSum += wt;
            y += 1;
        } catch (const std::exception& e) {
            throw std::runtime_error("Error in VBlur computation: " + std::string(e.what()));
        }
    }

    if (wtSum == 0.0f) {
        throw std::runtime_error("Weight sum is zero in VBlur");
    }
    return res / wtSum;
}

void Blur::VBlurIntoShmem(
    const std::shared_ptr<void>& inputTex,
    std::vector<glm::vec4>& vblurOut,
    const glm::ivec2& dstPx,
    int32_t xfetch,
    const glm::uvec2& groupId
) {
    try {
        glm::ivec2 srcPx = glm::ivec2(groupId) * glm::ivec2(static_cast<int32_t>(GROUP_WIDTH) * 2, 2)
            + glm::ivec2(xfetch - KERNEL_RADIUS, -KERNEL_RADIUS);
        
        if (static_cast<size_t>(xfetch) >= vblurOut.size()) {
            throw std::out_of_range("xfetch index out of bounds in VBlurIntoShmem");
        }
        
        vblurOut[static_cast<size_t>(xfetch)] = VBlur(inputTex, dstPx, srcPx);
    } catch (const std::exception& e) {
        throw std::runtime_error("Error in VBlurIntoShmem: " + std::string(e.what()));
    }
}

void Blur::BlurCs(
    const std::shared_ptr<void>& inputTex,
    const std::shared_ptr<void>& outputTex,
    std::vector<glm::vec4>& vblurOut,
    const glm::uvec3& px,
    const glm::uvec3& pxWithinGroup,
    const glm::uvec3& groupId
) {
    try {
        glm::uvec2 px2D = glm::uvec2(px.x, px.y);
        glm::uvec2 groupId2D = glm::uvec2(groupId.x, groupId.y);

        uint32_t xfetch = pxWithinGroup.x;
        while (xfetch < VBLUR_WINDOW_SIZE) {
            VBlurIntoShmem(inputTex, vblurOut, glm::ivec2(px2D), static_cast<int32_t>(xfetch), groupId2D);
            xfetch += GROUP_WIDTH;
        }

        ControlBarrier();

        glm::vec4 res(0.0f);
        float wtSum = 0.0f;

        for (int32_t x = 0; x <= KERNEL_RADIUS * 2; ++x) {
            float wt = GaussianWt(static_cast<float>(px2D.x), static_cast<float>(static_cast<int32_t>(px2D.x) * 2 + x - KERNEL_RADIUS));
            size_t index = static_cast<size_t>(pxWithinGroup.x * 2 + static_cast<uint32_t>(x));
            if (index >= vblurOut.size()) {
                throw std::out_of_range("Index out of bounds in BlurCs horizontal pass");
            }
            res += vblurOut[index] * wt;
            wtSum += wt;
        }

        if (wtSum == 0.0f) {
            throw std::runtime_error("Weight sum is zero in BlurCs horizontal pass");
        }
        res /= wtSum;

        WriteTexture(outputTex, px2D, res);
    } catch (const std::exception& e) {
        throw std::runtime_error("Error in BlurCs: " + std::string(e.what()));
    }
}

glm::vec4 Blur::FetchTexture(const std::shared_ptr<void>& texture, const glm::ivec2& coord) {
    if (!texture) {
        throw std::invalid_argument("Texture is null in FetchTexture");
    }
    return glm::vec4(0.0f);
}

void Blur::WriteTexture(const std::shared_ptr<void>& texture, const glm::uvec2& coord, const glm::vec4& value) {
    if (!texture) {
        throw std::invalid_argument("Texture is null in WriteTexture");
    }
}

void Blur::ControlBarrier() {
}

} // namespace tekki::rust_shaders