...
{
    return input_resolution.x >= min_render_extent.x && input_resolution.y >= min_render_extent.y &&
           input_resolution.x <= max_render_extent.x && input_resolution.y <= max_render_extent.y;
}

// DlssRenderer implementation
DlssRenderer::DlssRenderer(std::shared_ptr<Device> device, const glm::uvec2& input_resolution,
...
{
    spdlog::info("Initializing DLSS for input resolution [{}, {}] -> target resolution [{}, {}]", input_resolution.x,
...
DlssOptimalSettings DlssRenderer::get_optimal_settings_for_target_resolution(NVSDK_NGX_Parameter* ngx_params,
                                                                             const glm::uvec2& target_resolution,
                                                                             NVSDK_NGX_PerfQuality_Value quality_value)
{
    ...
    settings.optimal_render_extent = glm::uvec2(static_cast<uint32_t>(target_resolution.x * scale_factor),
                                                static_cast<uint32_t>(target_resolution.y * scale_factor));

    // Set reasonable bounds
    settings.min_render_extent = glm::uvec2(static_cast<uint32_t>(target_resolution.x * 0.3f),
                                            static_cast<uint32_t>(target_resolution.y * 0.3f));

    settings.max_render_extent = glm::uvec2(static_cast<uint32_t>(target_resolution.x * 0.9f),
                                            static_cast<uint32_t>(target_resolution.y * 0.9f));

    return settings;
}

std::shared_ptr<Image> DlssRenderer::render(vk::CommandBuffer command_buffer, std::shared_ptr<Image> input,
                                            ...
                                            const glm::uvec2& output_extent)
{
    ...
    // Create output image
    auto output = std::make_shared<Image>(device_, ImageDesc::new_2d(vk::Format::eR16G16B16A16Sfloat, output_extent)
...
NVSDK_NGX_Result DlssUtils::nv_sdk_ngx_vulkan_init(uint32_t application_id, const wchar_t* application_data_path,
...
NVSDK_NGX_Result DlssUtils::nv_sdk_ngx_vulkan_create_feature(void* command_list, int feature_id,
...
NVSDK_NGX_Result DlssUtils::nv_sdk_ngx_vulkan_evaluate_feature(void* command_list, NVSDK_NGX_Handle* feature_handle,
