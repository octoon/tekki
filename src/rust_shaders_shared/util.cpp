...
inline uint32_t Float3ToRgb9e5(const glm::vec3& rgb) {
    ...
    int32_t exp_shared = MyMax(-static_cast<int32_t>(RGB9E5_EXP_BIAS) - 1, FloorLog2Positive(maxrgb))
        + 1
        + static_cast<int32_t>(RGB9E5_EXP_BIAS);
...
inline glm::vec3 Rgb9e5ToFloat3(uint32_t v) {
    int32_t exponent = static_cast<int32_t>(BitfieldExtract(v, 0, RGB9E5_EXPONENT_BITS))
        - static_cast<int32_t>(RGB9E5_EXP_BIAS)
        - static_cast<int32_t>(RGB9E5_MANTISSA_BITS);
