...
| 阶段 | 交付内容 | 预估周期 |
| --- | --- | --- |
| M1 基础设施 | CMake 骨架、core 模块、后台空实现编译通过 | 2-3 周 |
| M2 Vulkan 后端 | 完成资源管理、命令录制、同步、Shader 编译 | 4-6 周 |
| M3 Render Graph | 图构建、资源追踪、Temporal 支持、单元测试 | 3-4 周 |
| M4 世界渲染器 | Mesh/实例/TLAS、关键渲染 pass、UI 挂钩 | 6-8 周 |
| M5 工具链 | Viewer、Asset Baker、Hello Demo | 3-4 周 |
| M6 着色器与回归 | 着色器迁移、自动化测试、性能调优 | 4-6 周 |
