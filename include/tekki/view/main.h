...
private:
    ...
    SimpleMainLoop kajiya_;

public:
    /**
     * Constructs a new AppState with persisted state and options
     */
    static AppState New(PersistedState persisted, const Opt& opt) {
     ...
     */
    void AddStandaloneMesh(const std::filesystem::path& path, float mesh_scale) {
        try {
...
private:
    AppState(PersistedState persisted, RuntimeState runtime, SimpleMainLoop kajiya)
        : persisted_(std::move(persisted))
        , runtime_(std::move(runtime))
        , kajiya_(std::move(kajiya)) {}
};
