...
private:
    ...
    private:
        std::string errorColor = "\x1B[31m";
        std::string warnColor = "\x1B[33m";
        std::string infoColor = "\x1B[32m";
        std::string debugColor = "\x1B[37m";
        std::string traceColor = "\x1B[90m";

    public:
        ColoredLevelConfig() = default;

        ColoredLevelConfig& Error(const std::string& color) {
            errorColor = color;
            return *this;
        }

        ColoredLevelConfig& Warn(const std::string& color) {
            warnColor = color;
            return *this;
        }

        ColoredLevelConfig& Info(const std::string& color) {
            infoColor = color;
            return *this;
        }

        ColoredLevelConfig& Debug(const std::string& color) {
            debugColor = color;
            return *this;
        }

        ColoredLevelConfig& Trace(const std::string& color) {
            traceColor = color;
            return *this;
        }
        ...
        std::string Color(LogLevel level) const {
    ...
    private:
        ...
        std::vector<std::pair<std::string, LogLevel>> levelOverrides;
    ...
    public:
        ...
        LogDispatch& Level(LogLevel level) {
            defaultLevel = level;
            return *this;
        }

        LogDispatch& LevelFor(const std::string& target, LogLevel level) {
            levelOverrides.emplace_back(target, level);
            return *this;
        }

        LogDispatch& Chain(std::ostream& stream) {
        ...
        LogDispatch& Chain(std::shared_ptr<std::ofstream> file) {
        ...
        void Apply() {
            // Apply would set up the global logger
        }
    };

public:
    static void SetUpLogging(LogLevel defaultLogLevel) {
