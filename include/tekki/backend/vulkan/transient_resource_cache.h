...
struct TransientBufferDesc {
    ...
    bool operator==(const TransientBufferDesc& other) const {
...
struct TransientImageDescHash {
    std::size_t operator()(const TransientImageDesc& desc) const {
        return std::hash<uint32_t>{}(desc.extent.width) ^
               std::hash<uint32_t>{}(desc.extent.height) ^
               std::hash<uint32_t>{}(desc.extent.depth) ^
               std::hash<int>{}(static_cast<int>(desc.format)) ^
               std::hash<uint32_t>{}(static_cast<uint32_t>(desc.usage)) ^
               std::hash<uint32_t>{}(desc.mip_levels) ^
...
public:
    ...
    // Buffer management
    std::shared_ptr<TransientBuffer> get_or_create_buffer(
        const TransientBufferDesc& desc,
        const std::string& name
    );

    // Image management
    std::shared_ptr<TransientImage> get_or_create_image(
...
private:
    ...
    // Buffer cache
    std::unordered_map<TransientBufferDesc, std::vector<std::shared_ptr<TransientBuffer>>, TransientBufferDescHash> buffer_cache_;

    // Image cache
    std::unordered_map<TransientImageDesc, std::vector<std::shared_ptr<TransientImage>>, TransientImageDescHash> image_cache_;
