...
struct BufferDesc {
    ...
    BufferDesc(size_t size, VkBufferUsageFlags usage, MemoryLocation memoryLocation, std::optional<uint64_t> alignment = std::nullopt)
...
class Device {
public:
    static std::shared_ptr<Buffer> CreateBufferImpl(
        VkDevice raw,
    ...
    void ImmediateDestroyBuffer(const std::shared_ptr<Buffer>& buffer) {
...
private:
    VkDevice Raw;
    std::shared_ptr<gpu_allocator::VulkanAllocator> GlobalAllocator;

    template<typename F>
    void WithSetupCommandBuffer(F&& func) {
