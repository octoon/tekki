...
class Device {
public:
    void RecordCrashMarker(const std::shared_ptr<CommandBuffer>& cb, const std::string& name) {
...
private:
    CrashMarkerNames CrashMarkerNamesObj;
    std::mutex CrashMarkerNamesMutex;
    std::shared_ptr<Buffer> CrashTrackingBuffer;
    // Other device members...
};
